...
app.post('/api/payment/submit-utr', (req, res) => {
    const { utr, planTier, amount, userPhone, userName } = req.body;

    if (!utr || !planTier) {
        return res.status(400).json({
            success: false,
            message: "UTR और Plan Tier अनिवार्य हैं।"
        });
    }
    ...
    const existing = payments.find(p => p.utr.toLowerCase() === cleanUtr.toLowerCase());
    if (existing) {
        return res.json({
            success: true,
    ...
    const newPayment = {
        id: 'PAY_' + Date.now(),
        utr: cleanUtr,
        planTier: planTier,       // उदा. BASIC_299 या PRO_399
    ...
    console.log(`[NEW PAYMENT] UTR: ${cleanUtr}, Plan: ${planTier}, User: ${userPhone}`);
...
// (Android App इसे समय-समय पर पोल कर सकती है)
// ==========================================
...
app.post('/api/admin/update-status', (req, res) => {
    ...
    // सुरक्षा के लिए सीक्रेट की (डिफ़ॉल्ट: myAdminSecret123)
...
// ==========================================
// 4. Admin API (नया): सभी पेमेंट्स JSON में लाना
// स्टैंडअलोन HTML डैशबोर्ड यही endpoint इस्तेमाल करेगा
// adminKey ?adminKey=... query param से या
// x-admin-key header से भेजी जा सकती है
// ==========================================
app.get('/api/admin/payments', (req, res) => {
    ...
    const payments = getPayments().reverse(); // नए पेमेंट सबसे ऊपर
    res.json({ success: true, payments });
});

// ==========================================
// 5. Admin Web Dashboard (सर्वर-रेंडर्ड, पुराना वाला - वैसे ही रहने दिया)
...
app.listen(PORT, () => {
    ...
    console.log(`🌐 Admin Panel (server-rendered): http://localhost:${PORT}/admin`);
    console.log(`🌐 Admin Panel (standalone HTML):  admin-dashboard.html`);
    console.log(`📲 API Update Check: GET  http://localhost:${PORT}/api/app/check-update`);
    console.log(`📲 API Submit: POST http://localhost:${PORT}/api/payment/submit-utr`);
    console.log(`📲 API Status: GET  http://localhost:${PORT}/api/payment/status/:utr`);
    console.log(`📲 API List (JSON): GET  http://localhost:${PORT}/api/admin/payments?adminKey=...`);
    console.log(`===================================================`);
});
            message: "UTR और Plan Tier अनिवार्य हैं।"
        });
